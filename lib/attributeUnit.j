...
struct hAttrUnit
	...
	// 硬直条，系统默认在允许的情况下会自动绑定，一般情况下不需要刻意使用
	...
	private static method triggerUnitDeathAction takes nothing returns nothing
        ...
        set heventBean.triggerKey = "dead"
        set heventBean.triggerUnit = u
        set heventBean.killer = killer
        call hevent.triggerEvent(heventBean)
        call heventBean.destroy()
        //@触发击杀事件
        set heventBean = hEventBean.create()
        set heventBean.triggerKey = "kill"
        set heventBean.killer = killer
        set heventBean.triggerUnit = killer
        set heventBean.targetUnit = u
        call hevent.triggerEvent(heventBean)
        call heventBean.destroy()
		set u = null
		set killer = null
	...
	private static method triggerInAction takes nothing returns nothing
		...
		if(isBind != true)then
			...
			if(GetUnitAbilityLevel(u,'Aloc')>0)then
				...
				//触发注册事件(全局)
				set heventBean = hEventBean.create()
				set heventBean.triggerKey = "register"
